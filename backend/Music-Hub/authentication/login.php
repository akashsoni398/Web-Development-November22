...
if(isset($_POST['submit'])) {
  ...
  if($email!='' && $pwd!='') {
    $sql_query = "SELECT count(*) AS usercount,password AS hash FROM users WHERE useremail='$email';";
    $result = mysqli_query($conn, $sql_query);
    $row = mysqli_fetch_array($result);
    if($row['usercount']==1 && password_verify($pwd,$row['hash'])==true){
      $sql_query = "SELECT username as name,id as id FROM users WHERE useremail='$email';";
      $result = mysqli_query($conn, $sql_query);
      $row = mysqli_fetch_array($result);
...
<div class="login-form">
  <form method="post" action=""> 
    ...
    <div class="content">
      ...
      <button type="submit" name="submit" class="link">SUBMIT</button><br>
